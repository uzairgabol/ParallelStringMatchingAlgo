...
void rabin_karp_omp(char **text, int num_lines, char *pattern, int m, FILE *output_file)
{
    unsigned long long patternHash = hash(pattern, m);

#pragma omp parallel for schedule(dynamic) shared(text, num_lines, pattern, m, output_file)
...
#pragma omp critical
                fprintf(output_file, "For Rabin-Karp Search OMP: Line %d, Index %d: pattern - (%s)\n", i + 1, j, pattern);
            }
...
int main()
{
    ...
    rabin_karp_omp(text, num_lines, pattern, m, output_file);
    ...
    printf("Total Execution Time Taken For Rabin-Karp OMP is: %f seconds\n\n", end_time - start_time);
