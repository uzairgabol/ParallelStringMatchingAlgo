...
void invertedArraySearch(char **text, int num_lines, char *pattern, int m, FILE *output_file)
{
    ...
    for (int i = 0; i < 256; ++i)
    {
        ...
        invertedArray[i][0] = -1; // Initialize line number to -1 for characters not found
        invertedArray[i][1] = -1; // Initialize index to -1 for characters not found
    }
...
int main()
{
    ...
    printf("Inverted-Array OMP Method\n");
    ...
    printf("Total Execution Time Taken For Inverted-Array OMP is: %f seconds\n\n", end_time - start_time);
